...
import { getTodayString } from "../goals/services/localStorageService";
...
export default function WeekView() {
  ...
  useEffect(() => {
    const loadWeekData = () => {
      ...
      const weekStart = dayjs(today).day(0); // 0 = Sunday
      const dates: string[] = [];
  ...
  const getWeeklyGoalProgress = (goal: GoalWithDailyStatus) => {
    ...
    const storageService = LocalStorageService.getInstance();
    const today = getTodayString();
    const weeklyGoals = storageService.getWeeklyGoalsForDate(today);
    const goalStatus = weeklyGoals.find((gs) => gs.id === goal.id);
